...
def get(x, y):
    global arr, size_x, size_y, offset_x, offset_y
    if x < -offset_x:
        return None
    if y < -offset_y:
        return None
...
def set(x, y, val):
    ...
    while x < -offset_x:
        arr.insert(0, [None for _ in arr[0]])
        offset_x += 1
        size_x += 1
    while y < -offset_y:
        for r in arr:
    ...
    while x > size_x - 1:
        arr.append([None for _ in arr[0]])
        size_x += 1
    while y > size_y - 1:
        for r in arr:
            r.append(None)
...
def print_arr(arr, format=None):
    ...
    for y in range(len(arr[0])):
        ...
        for x in range(len(arr)):
            line += ("%d" % (0 if arr[x][y] is None else arr[x][y])
                     ) if format is None else format[arr[x][y]]
        out += line + "\n"
    print(out, end="")
...
def walk(path, prev="", pos=None):
    ...
    for _ in range(l):
        ...
        if isinstance(c, list):
            for option in c:
                walk([option] + path, prev, [pos[0], pos[1]])
            break
        ...
        a = get(pos[0], pos[1])
        if a is None or len(a[0]) > len(prev):
            set(pos[0], pos[1], [prev, c])
...
def main():
    ...
    # print_arr([[" " if j is None else j[1] for j in i] for i in arr], {
    #     "N": "N",
    #     "E": "E",
    #     "S": "S",
    #     "W": "W",
    ...
    # m = 0
    # for l in [[0 if j is None else len(j[0]) for j in i] for i in arr]:
    #     m = max([m] + l)
    print(walk_len(input))
